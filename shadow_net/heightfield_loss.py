...
class H2I(nn.Module):
    ...
    def __init__(self, im_size, radius, device=3):
        ...
        self.buffer_vector , self.selecting_matrix = buffer_vector.cuda(device), selecting_matrix.cuda(device)
    ...
    def forward(self, heightfield):
        ...
        # if not (unroll_heights == unroll_heights2).all():
        #     a =5
        assert (unroll_heights == unroll_heights2).all()
        compare_heights, _ = unroll_heights2.max(dim=4)
        # image = 1 - self.clamp(compare_heights - heightfield, 0, 1)
        image = compare_heights - heightfield
        # image = heightfield - compare_heights
        return image
...
class HFLoss:
    def __init__(self, im_size, radius, gradient_weight, device=3):
        self.device = device
        self.h2i = H2I(im_size, radius, device)
        self.mse = nn.MSELoss().cuda(device)
        self.gradient_conv = self.init_grad_conv(device)
        self.gamma = gradient_weight
        self.zeros = torch.zeros(1, 1, im_size, im_size)

    def __call__(self, real_images, heightfield):
        loss = Variable(torch.zeros(1,1, requires_grad=True)).cuda(self.device)
        ...
        for i in range(real_images.size()[1]):
            rot_img = real_images.squeeze(0).select(1, i)
        ...
        if self.gamma:
            zeros = self.zeros.expand(heightfield.shape).cuda(self.device)
            loss += self.gamma * self.mse(self.gradient_conv(heightfield), zeros)
        return loss

    @staticmethod
    def init_grad_conv(device):
        filter_x = torch.Tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]]).cuda(device)
        filter_y = torch.Tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]).cuda(device)
        conv_x = nn.Conv2d(1, 1, kernel_size=3, stride=1, padding=1, bias=False).cuda(device)
        conv_y = nn.Conv2d(1, 1, kernel_size=3, stride=1, padding=1, bias=False).cuda(device)
        with torch.no_grad():
